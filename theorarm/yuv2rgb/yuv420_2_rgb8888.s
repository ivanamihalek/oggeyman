...
xloop1:
	...
	BNE	fix101 
return101:
	@ the last byte in r5 is 0, so repurpose it here for setting alpha to 1
	ORR	r5, r5, #255
	@ Store the bottom one first	
	ADD	r12,r0, r9
	
	STRB	r7,[r12,#2]		@ Store R -  store 1 byte (the least significant one I imagine)
	                                @            in addres given in r12, then increase r12 by 1
	
	MOV	r7, r7, ROR #22		@ rotate r7 right by 22 (and store back in r7)
	STRB	r7,[r12,#1]		@ Store G
	
	MOV	r7, r7, ROR #21
	STRB	r7,[r12]		@ Store B
	STRB	r5,[r12,#3]             @ last byte  is alpha 

	ADD     r12, r12, #4

	@ Then store the top one
	STRB	r6,[r0, #2]		@ Store R
