 ...
 ********************************************************************
  ...
  function:
    last mod: $Id: decode.c 16520 2009-08-26 12:31:38Z xiphmont $
...
int th_decode_packetin(th_dec_ctx *_dec,const ogg_packet *_op,
	...
	if (_op->bytes != 0) {
		...
		if (_dec->state.frame_type != OC_INTRA_FRAME
			...
			oc_memset_al_mult8(_dec->state.ref_frame_data[0], 0x80,
					yplane_sz + 2 * cplane_sz);
		} else {
			for (refi = 0;
		...
		_dec->state.granpos = (_dec->state.keyframe_num
				+ _dec->state.granpos_bias
