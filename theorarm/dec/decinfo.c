...
static int oc_dec_headerin(oc_pack_buf *_opb, th_info *_info, th_comment *_tc,
	...
	if (!(packtype & 0x80) && _info->frame_width > 0 && _tc->vendor != NULL
			&& *_setup != NULL) {
		return 0;
	}
...
int th_decode_headerin(th_info *_info, th_comment *_tc, th_setup_info **_setup,
		ogg_packet *_op) {
	oc_pack_buf opb;
