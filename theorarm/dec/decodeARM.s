...
@ Copyright (C) 2009 Robin Watts for Pinknoise Productions Ltd
	...
	.include "common.s"
	.include "offsetsARM.s"

	.global	oc_sb_run_unpack
	.global	oc_block_run_unpack
	.global	oc_dec_partial_sb_flags_unpack
	.global	oc_dec_coded_sb_flags_unpack
	.global	oc_dec_coded_flags_unpack

	.extern	theorapackB_lookARM@oc_pack_look
	.extern	theorapackB_read1ARM
	.extern	oggpack_adv@oc_pack_adv

oc_sb_run_unpack:
	STMFD	r13!,{r0,r4,r14}

	MOV	r1, #18
	BL	theorapackB_lookARM@oc_pack_look
	MOV	r1, #1		@ r1 = adv = 1
	MVN	r2, #0		@ r2 = sub = -1
	TST	r0, #0x20000	@ if (bits0x0x20000)
	MOVNE	r1, #3		@   r1 = adv = 3
	MOVNE	r2, #2		@   r2 = sub = 2
	TSTNE	r0, #0x10000	@   if (bits0x0x10000)
	MOVNE	r1, #4		@     r1 = adv = 4
	MOVNE	r2, #8		@     r2 = sub = 8
	TSTNE	r0, #0x08000	@     if (bits0x0x08000)
	MOVNE	r1, #6		@       r1 = adv = 6
	MOVNE	r2, #50		@       r2 = sub = 50
	TSTNE	r0, #0x04000	@       if (bits0x0x04000)
	MOVNE	r1, #8		@         r1 = adv = 8
	MOVNE	r2, #230	@         r2 = sub = 230
	TSTNE	r0, #0x02000	@         if (bits0x0x02000)
	MOVNE	r1, #10		@           r1 = adv = 10
	ADDNE	r2, r2, #974-230@           r2 = sub = 974
	TSTNE	r0, #0x01000	@           if (bits0x0x01000)
	MOVNE	r1, #18		@             r1 = adv = 18
	LDRNE	r2, =258014	@             r2 = sub = 258014
	RSB	r4, r1, #18
	RSB	r4, r2, r0, LSR r4@ (r4 = bits>>(18-adv))-sub
	LDR	r0, [r13],#4
	BL	oggpack_adv@oc_pack_adv
	MOV	r0, r4
...
oc_block_run_unpack:
	...
	MOV	r1, #2		@ r1 = adv = 2
	MVN	r2, #0		@ r2 = sub = -1
	TST	r0, #0x100	@ if (bits0x0x100)
	MOVNE	r1, #3		@   r1 = adv = 3
	MOVNE	r2, #1		@   r2 = sub = 1
	TSTNE	r0, #0x080	@   if (bits0x0x080)
	MOVNE	r1, #4		@     r1 = adv = 4
	MOVNE	r2, #7		@     r2 = sub = 7
	TSTNE	r0, #0x040	@     if (bits0x0x040)
	MOVNE	r1, #6		@       r1 = adv = 6
	MOVNE	r2, #49		@       r2 = sub = 49
	TSTNE	r0, #0x020	@       if (bits0x0x020)
	MOVNE	r1, #7		@         r1 = adv = 8
	MOVNE	r2, #109	@         r2 = sub = 109
	TSTNE	r0, #0x010	@         if (bits0x0x010)
...
odcfu_lp8:
	...
	MOV	r4, r4, LSL #4
	ORR	r4, r4, #0x8
  .if CODED_FULLY < CODED_PARTIALLY
	CMP	r5, #CODED_FULLY
